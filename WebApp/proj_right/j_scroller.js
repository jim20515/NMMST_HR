...
* Web address: http://www.brajinar.com                                        *
...
function Scroller(x, y, width, height, border, padding, layer) {
...
function scrollerCreate() {
  ...
  start = '<table border=0'
        ...
        + ' width=' + this.width
        + ' height=' + this.height + '>'
        + '<tr><td onmouseover="myScroller.stop();" onmouseout="myScroller.start();">'
        + '<font'
        + ' color="' + this.fgColor + '"'
        + ' face="' + this.fontFace + '"'
        + ' size=' + this.fontSize + '>';
...
//    document.body.insertAdjacentHTML("beforeEnd", str);		// 2005/09/21, fen mark
    this.layer.insertAdjacentHTML("beforeEnd", str);			// 2005/09/21, fen add
    if (!isMinIE5)
      window.scrollTo(x, y);
