...
public partial class sys_f_f02_p_f0201_02 : p_hrBase
{
    ...
    {
		// 初始化
		this.uf_InitializeQuery(null, this.indb_hlf201, null);
		this.uf_InitializeComponent(dwF, null, this.indb_hlf201, u_Edit_F);
        ...
        this.indb_hlf201.dv_View.Table.Columns["hlf201_sdatetime"].DefaultValue = DbMethods.uf_GetDateTime().ToString();
        this.indb_hlf201.dv_View.Table.Columns["hlf201_cancel"].DefaultValue = "N";
        this.indb_hlf201.dv_View.Table.Columns["hlf201_cway"].DefaultValue = "2";
        ...
        bt_choose1.Attributes["OnClick"] = "uf_OpenWindow('p_f0201_03.aspx?ReturnFormID=" + this.uf_GetFormID() + "&ReturnObjectID=" + dwF_hlf201_vid.ClientID + "&ReturnObjectID1=" + dwF_hlf201_vname.ClientID + "', '', 640, 420, 'no', 'no')";
	...
	private bool u_Edit_F_PreEndEdit(Control adwF, n_dbbase andb_Data, DataRow adr_Row, DataRowView adrv_Row)
	{
		...
		{
			// 取得流水號的系統編號–資料庫最大值+1
            adrv_Row["hlf201_lid"] = f01Project.uf_Get_SystemNo("hlf201.hlf201_lid", "Slog");
			dwF_hlf201_lid.Text = adrv_Row["hlf201_lid"].ToString();

		}

        if (!IPAddress.TryParse(dwF_hlf201_ip.Text, out IPtest))
        {
            this.uf_Msg(dwF_hlf201_ip.Text.Trim() + "不符合IP格式，請修正！");
            return false;
        }
	...
	protected override bool uf_DwF_DataAfter(Control acto_Child, ref string as_Value, DwActions a_Action, DataRowView adrv_Row)
	{
        ...
        {
            // 建檔方式
            if (acto_Child == dwF_hlf201_cway_c)
            {
                dwF_hlf201_cway_c.Text = uf_Dg_Bind(dwF_hlf201_cway.Items, "2");
                return true;
            }
            // 志工姓名
            if (acto_Child == dwF_hlf201_vname)
            {
                dwF_hlf201_vname.Text = uf_Dg_Bind("hmd201", adrv_Row["hlf201_vid"]);
                return true;
            }
            // 刷卡時間
            if (acto_Child == dwF_hlf201_sdatetime)
            {
                DateTime ldt_date;
                ListItem lli_Item;
                ldt_date = Convert.ToDateTime(adrv_Row["hlf201_sdatetime"]);
